...
public class MainWindow {
	...
	public Shell open(Display dpy) {
		...
		// set icon
		try {
			m_shell.setImage(new Image(m_display, getClass().getClassLoader()
					.getResource("images/picsi.png").openStream()));
		} catch (IOException e) {}
		...
		item.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				if (m_views.hasSecondView()) {
					...
					Object[] filterTypes =
						{ "Input", "Output" };
					int o = JOptionPane.showOptionDialog(null, "Choose the image to print",
							"Print", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE,
							null, filterTypes, filterTypes[0]);
					if (o > 0) {
						view = m_views.getView(false);
						doc = m_views.getDocument(false);
					}
	...
	private void createHelpMenu(Menu menuBar) {
		...
		item.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				box.setMessage("Copyright (c) "
						+ calendar.get(Calendar.YEAR)
						+ " FHNW\nInstitute of Mobile and Distributed Systems\nBrugg-Windisch\nSwitzerland\nwww.fhnw.ch/imvs\nVersion "
						+ PicsiSWT.APP_VERSION);
				box.open();
			}
		});
	}
	...
	private boolean saveFile(boolean saveAs) {
		...
		try {
			...
			showErrorDialog("saving", (si != null) ? si.filename : m_views.getDocument(false)
					.getFileName(), e);
			return false;
		} finally {
			m_shell.setCursor(null);
	...
	private void setTitle(String filename, int filetype) {
		m_shell.setText(PicsiSWT.createMsg(PicsiSWT.APP_NAME + " - {0} ({1})", new Object[]
			{ filename, PicsiSWT.fileTypeString(filetype) }));
	}

	private void setIcon(Item item, String resourceName) {
		try {
			item.setImage(new Image(m_display, getClass().getClassLoader()
