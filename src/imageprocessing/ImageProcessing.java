		...
		for (final ImageMenuItem item : m_menuItems) {
			...
			mi.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent event) {
					Image output = null;
					try {
						output = item.m_process.run(m_views.getFirstImage(),
								m_views.getFirstimageType());
					} catch (Throwable e) {
						String location = item.m_text.substring(0, item.m_text.indexOf('\t'))
								.replace("&", "");
			...
			for (int x = 0; x < outData.width; x++) {
				...
				if (imageType == PicsiSWT.IMAGE_TYPE_GRAY) {
					...
					outData.setPixel(
							x,
							y,
							ImageProcessing.clamp8(a0 * b0 * A + a1 * b0 * B + a0 * b1 * C + a1
									* b1 * D));
				} else {
					// RGB: bilinear interpolation
					RGB cA = inData.palette.getRGB(A), cB = inData.palette.getRGB(B), cC = inData.palette
							.getRGB(C), cD = inData.palette.getRGB(D);

					RGB cG = new RGB(ImageProcessing.clamp8(a0 * b0 * cA.red + a1 * b0 * cB.red
							+ a0 * b1 * cC.red + a1 * b1 * cD.red), ImageProcessing.clamp8(a0 * b0
							* cA.green + a1 * b0 * cB.green + a0 * b1 * cC.green + a1 * b1
							* cD.green), ImageProcessing.clamp8(a0 * b0 * cA.blue + a1 * b0
							* cB.blue + a0 * b1 * cC.blue + a1 * b1 * cD.blue));
				...
				for (int u = 0; u < outData.width; u++) {
					...
					for (int j = 0; j < filter.length; j++) {
						...
						if (v0 < 0)
							v0 = -v0;
						if (v0 >= inData.height)
							v0 = 2 * inData.height - v0 - 1;
						for (int i = 0; i < filter.length; i++) {
							int u0 = u + i - fSizeD2;
							if (u0 < 0)
								u0 = -u0;
							if (u0 >= inData.width)
								u0 = 2 * inData.width - u0 - 1;
	...
	public static boolean isValidAllRGB(ImageData outData) {
		...
		for (int v = 0; v < outData.height; v++) {
			for (int u = 0; u < outData.width; u++) {
				...
				if (used[rgb.red][rgb.green][rgb.blue]) { return false; }
